...
- Run `pnpm lint --max-warnings 0` **and** `pnpm format`
  ...
  if [ -d tests ] && [ "$(ls -A tests 2>/dev/null)" ]; then pnpm test; fi
````

*(skip if the `tests/` directory is empty).*
...
Husky triggers **lint-staged** which *auto-fixes* and blocks on remaining errors.
...
An OpenAI agent **MUST** execute the following steps *before* returning its final answer:
...
3. **If any command exits non-zero**, iteratively amend the code *until* all commands succeed (max 3 internal iterations).

4. Return the final diff or files *only after all checks pass*. If still failing after 3 iterations, reply with an error report **instead of broken code**.
...
**Follow every MUST in this document. Passing lint, format, tests, and type-check *before* committing is non-negotiable.**
